...
const {image,answerOptions,questionNumber, id} = languageTestQuestionsOne[indexLanguageTest];
...
function nextQuestionTest(){
    ...
    return (
       ...
       <section className='language-test-two'>
         ...
         <section className='language-test-one-container'>
	...
	{answerOptions.map((answerOption) => (
		<button onClick={() => handleAnswerClickTest(answerOption.isCorrect)}className='answer-btn'>{answerOption.answerText}</button>
	))}
    <p>Question {questionNumber}/30</p>
      <button onClick={() => nextQuestionTest()} class="next-btn">Next</button>
</div>
          </section> )}
        </>
    )
